...
class DictGenerator:
    ...
    def structure_dict_xml(self, huns_dict):
        ...
        start_root_element ='<wordlist>\n'
        for word in huns_dict:

            part_of_child_element = '<w f='
            if "/" in word:
                splitted_str = word.split("/")
                values_of_child_element = '"'+splitted_str[1].rstrip("\n\r")+'">'+splitted_str[0].rstrip("\n\r")
            else:
                values_of_child_element = '"">'+word.rstrip("\n\r")
            end_of_child_element = '</w>\n'
            child_element += part_of_child_element + values_of_child_element + end_of_child_element 

        end_root_element = '</wordlist>'
    ...
    def write_xml_doc_to_file(self, huns_dict):
        f = open( self.output_path + '/dictionaries.xml','w')
